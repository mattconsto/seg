	...
	private static void setCustomContextMenu(TableView<?> table) {
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if (node instanceof TableHeaderRow) {
				...
				// if the preferred height isn't set, then the table header
				// would disappear if there are no visible columns
				// and with it the table menu button
				// by setting the preferred height the header will always be
				// visible
				// note: this may need adjustments in case you have different
				// heights in columns (eg when you use grouping)
