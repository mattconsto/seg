...
public class GraphUpdater  {
	...
	public GraphUpdater() {
        ...
        public void runUpdater(Filter f, String m, BounceFilter b, LineChart<Date, Number> lc, HashMap<String, XYChart.Series<Date, Number>> gd) {
             
                GraphThread s =   new GraphThread(f, m, b, lc, gd ); 
                executor.execute(s);
        ...
        public class GraphThread implements Runnable {
          ...
          public GraphThread(Filter f, String m, BounceFilter b, LineChart<Date, Number> lc, HashMap<String, XYChart.Series<Date, Number>> gd){
              this.filter = f;
              this.metric = m;
              this.bounceFilter = b;
              this.lineChart = lc;
          ...
          private void showHistogram(XYChart.Series<Date, Number> series) {
		...
		for(int i = 0; i < buckets; i++) {
			histogram_series.getData().add(new XYChart.Data<String, Number>(Integer.toString((int) (minimum + i * segment)), data[i]));
		}
	...
	public void run() {
            ...
            try {
                ...
                if (!graphData.containsKey(key)) {
                    ...
                    System.err.println("added grapg data " + key);
                    data.setName(key);
                    int last = 0;
